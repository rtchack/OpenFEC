...
void *of_chk_alloc     (OF_UINT32	n,		/* Number of elements */
			OF_UINT32	size)		/* Size of each element */
{
	void *p;
...
OF_INT32 of_intio_read (FILE *f)		/* File to read from */
{
	OF_UINT8 b[4];
	OF_INT32 top;
	OF_INT32 i;
	...
	top = b[3] > 127 ? (OF_INT32) b[3] - 256 : b[3];
	OF_EXIT_FUNCTION
...
void of_intio_write	(FILE	*f,	/* File to write to */
			 OF_INT32	v)	/* Value to write to file */
{
	OF_UINT8 b;
	OF_INT32 i;
...
void of_print_composition (char* symbol, OF_UINT32 size)
{
	OF_UINT32 i;
