...
of_mod2dense *of_mod2dense_allocate (OF_UINT32	n_rows,		/* Number of rows in matrix */
...
OF_UINT32 of_mod2dense_write (FILE *f, of_mod2dense *m)
{
	OF_ENTER_FUNCTION
	OF_UINT32 j, k;
...
OF_UINT32 of_mod2dense_get (of_mod2dense	*m, 	/* Matrix to get element from */
			 OF_UINT32		row,	/* Row of element (starting with zero) */
			 OF_UINT32		col)	/* Column of element (starting with zero) */
{
	//OF_ENTER_FUNCTION
#ifdef OF_DEBUG
...
OF_INT32 of_mod2dense_set (of_mod2dense	*m, 	/* Matrix to modify element of */
			OF_UINT32		row,		/* Row of element (starting with zero) */
			OF_UINT32		col,		/* Column of element (starting with zero) */
			OF_UINT32		value)		/* New value of element (0 or 1) */
{
	of_mod2word *w;
...
OF_UINT32 of_mod2dense_flip (of_mod2dense	*m, 	/* Matrix to flip element in */
			  OF_UINT32	row,	/* Row of element (starting with zero) */
			  OF_UINT32	col)	/* Column of element (starting with zero) */
{
	of_mod2word *w;
...
OF_UINT32 of_mod2dense_triangularize
(of_mod2dense *m,		/* The matrix to find the inverse of (destroyed) */
 of_mod2dense *r		/* Place to store the inverse */
)
{
...
OF_UINT32 of_mod2dense_invert_selected
(of_mod2dense *m,		/* Matrix from which to pick a submatrix to invert */
 of_mod2dense *r,		/* Place to store the inverse */
 OF_UINT32 *rows,		/* Set to indexes of rows used and not used */
 OF_UINT32 *cols		/* Set to indexes of columns used and not used */
)
{
	OF_ENTER_FUNCTION
	of_mod2word *s, *t;
	OF_UINT32  R;
	OF_UINT32 i, j, k, n,  w, k0, b0, n2, c;
...
OF_UINT32 of_mod2dense_forcibly_invert
(of_mod2dense *m, 	/* The matrix to find the inverse of (destroyed) */
 of_mod2dense *r,		/* Place to store the inverse */
 OF_UINT32 *a_row,		/* Place to store row indexes of altered elements */
 OF_UINT32 *a_col		/* Place to store column indexes of altered elements */
)
{
	OF_ENTER_FUNCTION
	//  of_mod2word *s, *t;
...
{
	OF_UINT32		i;
	OF_UINT32		n_w;	// number of of_mod2word in the row
...
OF_UINT32 of_mod2dense_row_weight (of_mod2dense *m, OF_UINT32 i)
{
	OF_UINT32		weight = 0;
...
#define SLOW_HAMMING_WEIGHT_CALCULATION
	...
	OF_UINT32		j;
	OF_UINT32		nwords;			/* number of of_mod2word in a row */
	//OF_UINT32		weight1 = 0;
...
#else
	weight = of_hweight_array ((OF_UINT32 *) m->row[i], of_mod2dense_cols (m));
#endif
...
OF_UINT32 of_mod2dense_row_weight_ignore_first (of_mod2dense *m, OF_UINT32 i, OF_UINT32 nb_ignore)
{
	OF_UINT32		weight = 0;
	//OF_UINT32	nwords;
	OF_UINT32		start0;
	...
	OF_UINT32 offset32 = nb_ignore >> of_mod2_wordsize_shift; // number of 32bits words that must be ignored
	OF_UINT32 rem = of_mod2dense_cols (m) - (offset32 << of_mod2_wordsize_shift);     // number of remaining bits to consider

	weight = of_hweight_array ((OF_UINT32 *) & m->row[i][offset32], rem);
	OF_EXIT_FUNCTION
	return weight;
}


OF_UINT32 of_mod2dense_col_weight (of_mod2dense *m, OF_UINT32 i)
{
	OF_ENTER_FUNCTION
	OF_UINT32 weight = 0;
...
void of_mod2dense_print_stats (FILE *f, of_mod2dense *m)
{
	...
	OF_UINT32		max_entry_per_row = 0;
	OF_UINT32		min_entry_per_row = 9999999;
	OF_UINT32		nb_entry_row[ROW_MAX_ENTRY];
	OF_UINT32		nb_entry_current_row = 0;
	OF_UINT32		ctr = 0;
	...
	OF_UINT32		max_entry_per_col = 0;
	OF_UINT32		min_entry_per_col = 9999999;
	OF_UINT32		nb_entry_col[COL_MAX_ENTRY];
	OF_UINT32		nb_entry_current_col = 0;
...
void of_mod2dense_xor_rows(of_mod2dense *m, OF_UINT16 from, OF_UINT16 to)
{
	OF_UINT32		i;
	of_mod2word	*f;
	of_mod2word	*t;
