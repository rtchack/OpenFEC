...
of_mod2sparse *of_mod2sparse_allocate  (OF_UINT32	n_rows,		/* Number of rows in matrix */
					OF_UINT32	n_cols)		/* Number of columns in matrix */
{
	//OF_ENTER_FUNCTION
	of_mod2sparse *m;
	of_mod2entry *e;
...
void of_mod2sparse_copyrows    (of_mod2sparse	*m,	/* Matrix to copy */
				of_mod2sparse	*r,	/* Place to store copy of matrix */
				OF_UINT32		*rows)	/* Indexes of rows to copy, from 0 */
{
	of_mod2entry *e;
	OF_INT32 i;
...
void of_mod2sparse_copycols    (of_mod2sparse	*m,		/* Matrix to copy */
				of_mod2sparse	*r,		/* Place to store copy of matrix */
				OF_UINT32		*cols)		/* Indexes of columns to copy, from 0 */
{
	of_mod2entry *e;
	OF_INT32 j;
				 ...
				 OF_UINT32		nb_par)
{
	...
	OF_INT32		max_entry_per_row = 0;
	OF_INT32		min_entry_per_row = 9999999;
	OF_INT32		nb_entry_row[ROW_MAX_ENTRY];
	OF_INT32		nb_entry_current_row = 0;
	OF_INT32		ctr = 0;

	/* cols stats*/
	float		aver_nb_entry_per_col = 0;
	OF_INT32		max_entry_per_col = 0;
	OF_INT32		min_entry_per_col = 9999999;
	OF_INT32		nb_entry_col[COL_MAX_ENTRY];
	OF_INT32		max_entry_per_data_col = 0;
	OF_INT32		min_entry_per_data_col = 9999999;
	OF_INT32		max_entry_per_parity_col = 0;
	OF_INT32		min_entry_per_parity_col = 9999999;
	OF_INT32		nb_entry_data_col[COL_MAX_ENTRY];
	OF_INT32		nb_entry_parity_col[COL_MAX_ENTRY];
	OF_INT32		nb_entry_current_col = 0;
...
OF_UINT32 of_mod2sparse_write_human_readable (FILE			*f,
					   of_mod2sparse	*m,
					   OF_UINT32		nb_source,
					   OF_UINT32		nb_parity)
{
	of_mod2entry *e;
	OF_INT32 i, col_idx, c;
	char buffer [20];
	OF_INT32 str_length;
...
(of_mod2sparse *m,	/* The sparse matrix, with M rows and N columns */
 OF_UINT8 *u,		/* The input vector, N long */
 OF_UINT8 *v		/* Place to store the result, M long */
)
{
	OF_ENTER_FUNCTION
	of_mod2entry *e;
...
OF_UINT32 mod2sparse_count_row
(of_mod2sparse *m,
 OF_UINT32 row
)
{
...
OF_UINT32 mod2sparse_count_col
(of_mod2sparse *m,
 OF_UINT32 col
)
{
...
(of_mod2sparse *m1,	/* Matrix containing row to add to */
 OF_UINT32 row1,		/* Index in this matrix of row to add to */
 of_mod2sparse *m2,	/* Matrix containing row to add from */
 OF_UINT32 row2		/* Index in this matrix of row to add from */
)
{
	OF_ENTER_FUNCTION
...
(of_mod2sparse *m1,	/* Matrix containing column to add to */
 OF_UINT32 col1,		/* Index in this matrix of column to add to */
 of_mod2sparse *m2,	/* Matrix containing column to add from */
 OF_UINT32 col2		/* Index in this matrix of column to add from */
)
{
	OF_ENTER_FUNCTION
	of_mod2entry *f1, *f2, *ft;
...
OF_UINT32 mod2sparse_decomp
(of_mod2sparse *A,	/* Input matrix, M by N */
 OF_UINT32 K,		/* Size of sub-matrix to find LU decomposition of */
 of_mod2sparse *L,	/* Matrix in which L is stored, M by K */
 of_mod2sparse *U,	/* Matrix in which U is stored, K by N */
 OF_UINT32 *rows,		/* Array where row indexes are stored, M long */
 OF_UINT32 *cols,		/* Array where column indexes are stored, N long */
 mod2sparse_strategy strategy, /* Strategy to follow in picking rows/columns */
 OF_UINT32 abandon_number,	/* Number of columns to abandon at some point */
 OF_UINT32 abandon_when	/* When to abandon these columns */
)
{
	OF_ENTER_FUNCTION
	OF_UINT32 *rinv = NULL, *cinv = NULL, *acnt = NULL, *rcnt = NULL;
	of_mod2sparse *B = NULL;
	OF_UINT32 M, N;

	of_mod2entry *e = NULL, *f = NULL, *fn = NULL, *e2 = NULL;
	OF_INT32 i = 0, j = 0, k = 0, cc = 0, cc2 = 0, cc3 = 0, cr2 = 0, pr = 0;
	OF_INT32 found, nnf;
	...
	rinv = (OF_UINT32*) of_calloc (M, sizeof * rinv);
	cinv = (OF_UINT32*) of_calloc (N, sizeof * cinv);

	if (abandon_number > 0)
	{
		acnt = (OF_UINT32*) of_calloc (M + 1, sizeof * acnt);
	}

	if (strategy == Mod2sparse_minprod)
	{
		rcnt = (OF_UINT32*) of_calloc (M, sizeof * rcnt);
	}
...
OF_UINT32 mod2sparse_forward_sub
(of_mod2sparse *L,	/* Matrix that is lower triangular after reordering */
 OF_UINT32 *rows,		/* Array of indexes (from 0) of rows for new order */
 OF_UINT8 *x,		/* Vector on right of equation, also reordered */
 OF_UINT8 *y		/* Place to store solution */
)
{
	OF_ENTER_FUNCTION
	OF_INT32 K, i, j, ii, b, d;
	of_mod2entry *e;
...
OF_UINT32 mod2sparse_backward_sub
(of_mod2sparse *U,	/* Matrix that is upper triangular after reordering */
 OF_UINT32 *cols,		/* Array of indexes (from 0) of columns for new order */
 OF_UINT8 *y,		/* Vector on right of equation */
 OF_UINT8 *z		/* Place to store solution, also reordered */
)
{
	OF_ENTER_FUNCTION
	OF_INT32 K, i, j, ii, b, d;
...
//
OF_UINT32 of_mod2sparse_swap_rows (of_mod2sparse	* m,  // Matrix in which we swap two rows
				OF_UINT32          row0, // swap row e0 and row e1
				OF_UINT32          row1,
				of_mod2sparse	*__swap,
				of_mod2entry	** __links,
				of_mod2entry	**__parsing)
{
	OF_UINT32 row_weight;
	OF_UINT32 __col;
...
//
OF_UINT32 of_mod2sparse_xor_rows  (of_mod2sparse	*m,    // Matrix in which we swap two rows
				OF_UINT32          row0, // xor row e0 and row e1, and set the result in row1
				OF_UINT32          row1,
				of_mod2entry	**__links,
				of_mod2entry	**__parsing)
{
...
bool of_mod2sparse_empty_row (of_mod2sparse * m, OF_UINT32 row)
{
	OF_ENTER_FUNCTION
...
bool of_mod2sparse_empty_col (of_mod2sparse * m, OF_UINT32 col)
{
	OF_ENTER_FUNCTION
	if (m != NULL)
	{
...
OF_UINT32 of_mod2sparse_weight_row (of_mod2sparse * m, OF_UINT32 row)
{
	OF_ENTER_FUNCTION
	of_mod2entry * __e;
	OF_INT32         row_weight;
...
#if 0
OF_UINT32 mod2sparse_weight_col (of_mod2sparse * m, OF_UINT32 col)
{
	OF_ENTER_FUNCTION
	of_mod2entry * __e;
	OF_INT32         col_weight;
...
void of_mod2sparse_copy_filled_matrix  (of_mod2sparse	*m,
					of_mod2sparse	*r,
					OF_UINT32		*index_rows,
					OF_UINT32		*index_cols)
{
	of_mod2entry	*e;
	OF_UINT32		i;
